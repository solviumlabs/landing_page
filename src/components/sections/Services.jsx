import { useState } from 'react';
import { Cloud, Cpu, Layout, Zap, Code2, Layers, CheckCircle2, ChevronRight } from 'lucide-react';
import SectionWrapper from '@components/common/SectionWrapper';
import Badge from '@components/common/Badge';
import { SERVICES, SERVICES_SECTION } from '@data/services';
import { COMPANY } from '@data/meta';
import { useLang } from '@utils/LangContext';

const ICON_MAP = { Cloud, Cpu, Layout, Zap, Code2, Layers };

const WhatsAppIcon = ({ className }) => (
  <svg 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    className={className}
    xmlns="http://www.w3.org/2000/svg"
  >
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
  </svg>
);

export default function Services() {
  const [active, setActive] = useState(0);
  const { t, lang } = useLang();

  const getWhatsAppLink = (serviceTitle) => {
    const phone = COMPANY.phone.replace(/\D/g, '');
    const message = lang === 'es' 
      ? `Hola Solvium, me interesa información sobre: ${serviceTitle}`
      : `Hello Solvium, I'm interested in information about: ${serviceTitle}`;
    return `https://wa.me/${phone}?text=${encodeURIComponent(message)}`;
  };

  return (
    <SectionWrapper id="servicios">
      {/* Header */}
      <div className="text-center mb-16 space-y-4">
        <Badge icon={Layers}>{t(SERVICES_SECTION.badge)}</Badge>
        <h2 className="text-4xl lg:text-5xl font-extrabold text-theme">
          {t(SERVICES_SECTION.title)}{' '}
          <span className="gradient-text">{t(SERVICES_SECTION.highlight)}</span>
        </h2>
        <p className="text-lg text-theme-secondary max-w-xl mx-auto">
          {t(SERVICES_SECTION.subtitle)}
        </p>
      </div>

      {/* Grid de tarjetas */}
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {SERVICES.map((service, idx) => {
          const Icon = ICON_MAP[service.icon] ?? Code2;
          const isActive = active === idx;
          const serviceTitle = t(service.title);

          return (
            <div
              key={service.id}
              onMouseEnter={() => setActive(idx)}
              className={`group relative rounded-2xl p-8 transition-all duration-300 cursor-default glass-card flex flex-col
                ${isActive ? 'glass-card-active scale-[1.02]' : ''}`}
            >
              <div className="flex-grow">
                {/* Ícono */}
                <div
                  className={`w-14 h-14 rounded-xl flex items-center justify-center mb-6
                    transition-all duration-300
                    ${isActive
                      ? 'gradient-brand shadow-xl shadow-brand-500/35'
                      : 'bg-brand-500/10 group-hover:bg-brand-500/15'
                    }`}
                >
                  <Icon className={`w-7 h-7 ${isActive ? 'text-white' : 'text-brand-400'}`} />
                </div>

                <h3 className="text-xl font-bold mb-3 text-theme">{serviceTitle}</h3>
                <p className="text-theme-secondary text-sm leading-relaxed mb-6">
                  {t(service.description)}
                </p>

                {/* Features */}
                <ul className="space-y-2 mb-8">
                  {service.features.map((feat, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm">
                      <CheckCircle2 className="w-4 h-4 text-brand-400 flex-shrink-0" />
                      <span className="text-theme-secondary">{t(feat)}</span>
                    </li>
                  ))}
                </ul>
              </div>

              {/* WhatsApp Link */}
              <a
                href={getWhatsAppLink(serviceTitle)}
                target="_blank"
                rel="noopener noreferrer"
                className={`mt-auto flex items-center justify-center gap-2 py-3 px-4 rounded-xl text-sm font-bold
                  transition-all duration-300 border
                  ${isActive 
                    ? 'bg-brand-500/10 border-brand-500/50 text-brand-400 shadow-lg shadow-brand-500/5' 
                    : 'border-theme text-theme-secondary hover:border-brand-500/30 hover:text-brand-400'}`}
              >
                <WhatsAppIcon className="w-4 h-4" />
                {t(SERVICES_SECTION.more)}
              </a>
            </div>
          );
        })}
      </div>
    </SectionWrapper>
  );
}
